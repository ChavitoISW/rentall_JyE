...
async function handlePut(req: NextApiRequest, res: NextApiResponse, id: string) {
  ...
  return res.status(200).json({ success: true, message: 'Detalle actualizado correctamente' });
}
...
async function handleDelete(req: NextApiRequest, res: NextApiResponse, id: string) {
  ...
  db.prepare('DELETE FROM detalle_hoja_ruta WHERE id_detalle_hoja_ruta = ?').run(id);

  return res.status(200).json({ success: true, message: 'Detalle eliminado correctamente' });
}
